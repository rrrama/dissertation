...
class Config(BaseConfig):
    ...
    to_mlp: bool = True
    to_head: bool = False
    stochastic_layer_threshold: float = 0.0
...
class GPT(BaseModel):
    def __init__(self, config: Config) -> None:
        ...
        self.lora_C = nn.Parameter(torch.zeros(4, config.n_layer, config.n_head, config.r))

        if self.config.stochastic_layer_threshold:
            print("*"*20)
            print(f"Using Stochastic layer subsampling, threshold: {self.config.stochastic_layer_threshold}")
    ...
    def forward(
        ...
        if self.config.stochastic_layer_threshold:
            subsampled_layers = max(0,int(self.config.stochastic_layer_threshold*len(self.transformer.h))-2)#np.random.rand(1).item()*2*
            #subsampled_layers = int(self.config.stochastic_layer_threshold)
        else:
            subsampled_layers = 0
        with torch.no_grad():
            for block_idx  in range(subsampled_layers):
                block = self.transformer.h[block_idx]
                dQ = torch.cat([self.lora_A @  torch.diag(self.lora_C[0,block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
                dK = torch.cat([self.lora_A @  torch.diag(self.lora_C[1, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
                dV = torch.cat([self.lora_A @  torch.diag(self.lora_C[2, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
                dP = torch.cat([self.lora_A @  torch.diag(self.lora_C[3, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
                x = block(x, cos, sin, mask, input_pos, dQ, dK, dV, dP)
        for block_idx  in range(len(self.transformer.h)-subsampled_layers):
            block = self.transformer.h[block_idx+subsampled_layers]
            dQ = torch.cat([self.lora_A @  torch.diag(self.lora_C[0,block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
            dK = torch.cat([self.lora_A @  torch.diag(self.lora_C[1, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
            dV = torch.cat([self.lora_A @  torch.diag(self.lora_C[2, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
            dP = torch.cat([self.lora_A @  torch.diag(self.lora_C[3, block_idx, head_idx])@self.lora_B for head_idx in range(self.config.n_head)], dim=1)
