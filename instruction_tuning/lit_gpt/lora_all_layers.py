...
class GPT(BaseModel):
    def __init__(self, config: Config) -> None:
        ...
        self.lora_A_Q = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.n_embd, config.r), a=math.sqrt(5)))
        self.lora_B_Q = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.r, head_dim), a=math.sqrt(5)))
        self.lora_C_Q = nn.Parameter(torch.zeros(config.n_layer, config.n_head, config.r))
        # K
        self.lora_A_K = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.n_embd, config.r), a=math.sqrt(5)))
        self.lora_B_K = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.r, head_dim), a=math.sqrt(5)))
        self.lora_C_K = nn.Parameter(torch.zeros (config.n_layer, config.n_head, config.r))
        # V
        self.lora_A_V = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.n_embd, config.r), a=math.sqrt(5)))
        self.lora_B_V = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.r, head_dim), a=math.sqrt(5)))
        self.lora_C_V = nn.Parameter(torch.zeros(config.n_layer, config.n_head, config.r))
        # P
        self.lora_A_P = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.n_embd, config.r), a=math.sqrt(5)))
        self.lora_B_P = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(config.r, head_dim), a=math.sqrt(5)))
        self.lora_C_P = nn.Parameter(torch.zeros(config.n_layer, config.n_head, config.r))
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self.transformer.h):
            ...
            dQ = torch.cat([self.lora_A_Q @  torch.diag(self.lora_C_Q[block_idx, head_idx])@self.lora_B_Q for head_idx in range(self.config.n_head)], dim=1)
            ...
            dK = torch.cat([self.lora_A_K @  torch.diag(self.lora_C_K[block_idx, head_idx])@self.lora_B_K for head_idx in range(self.config.n_head)], dim=1)
            dV = torch.cat([self.lora_A_V @  torch.diag(self.lora_C_V[block_idx, head_idx])@self.lora_B_V for head_idx in range(self.config.n_head)], dim=1)
            dP = torch.cat([self.lora_A_P @  torch.diag(self.lora_C_P[block_idx, head_idx])@self.lora_B_P for head_idx in range(self.config.n_head)], dim=1)
