...
class Framework:
    ...
    def train(self, train_samples, eval_samples):
        ...
        step_count = 0
        total_time_200_steps = 0
        total_samples_200_steps = 0
        peak_memory = 0
        total_flops = 0
        total_macs= 0
        ...
        for batch in trainer.get_train_dataloader():
            ...
            if step_count <= 100:
                # Skip timing for the first 100 steps
                trainer.training_step(self.model, batch)
                continue
            ...
            # Time measurement and GPU memory tracking
            ...
            flops, macs, params = calculate_flops(
                ...
                transformer_tokenizer=self.tokenizer,  # Assuming you have a tokenizer
            ...
            batch_peak_memory = max_memory_allocated() / 1024 ** 3  # Convert bytes to GB
            
            # Update metrics for the last 200 steps
            total_time_200_steps += batch_time
            total_samples_200_steps += len(batch["input_ids"])
            peak_memory = max(peak_memory, batch_peak_memory)
            total_flops += float(flops.split()[0])
            total_macs += float(macs.split()[0])

        # Calculate average metrics for the 200 measured steps
        avg_time_per_sample = total_time_200_steps / total_samples_200_steps if total_samples_200_steps else float("inf")
        avg_time_per_step = total_time_200_steps / 200 if step_count > 100 else float("inf")
        avg_flops_per_sample = total_flops / total_samples_200_steps if total_samples_200_steps else float("inf")
        avg_macs_per_sample = total_macs / total_samples_200_steps if total_samples_200_steps else float("inf")
        # Log metrics for the last 200 steps
        wandb.log({
            "average_time_per_sample": avg_time_per_sample,
            "average_time_per_step": avg_time_per_step,
            "average_flops_per_sample": avg_flops_per_sample,
            "average_macs_per_sample": avg_macs_per_sample,
...
def main():
    ...
    wandb.init(project="loretta-llama-profile", name=wandb_run_name)
