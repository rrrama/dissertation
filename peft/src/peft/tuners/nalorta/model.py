...
class NALorTaModel(BaseTuner):
    ...
    Args:
        ...
        config ([`NALorTaConfig`]): The configuration of the Lora model.
    ...
    Example:
        ...
        >>> from peft import LoraModel, NALorTaConfig
        ...
        >>> import torch
        >>> import transformers
        >>> from peft import NALorTaConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    ...
    **Attributes**:
        ...
        - **peft_config** ([`NALorTaConfig`]): The configuration of the Lora model.
    """
    ...
    def _get_arch_dims(config):
        """Resolve architecture dimensions across differing config naming schemes.

        Standard HF models (e.g. Llama) expose ``num_hidden_layers`` / ``num_attention_heads`` /
        ``hidden_size``. Some custom models such as LLaDA (``GSAI-ML/LLaDA-8B-Base``) instead expose
        ``n_layers`` / ``n_heads`` / ``d_model``. Returns ``(num_hidden_layers, num_attention_heads,
        hidden_size)``.
        """
        ...
        def _resolve(*names):
            for name in names:
                ...
                if value is not None:
                    return value
            raise AttributeError(f"Could not resolve architecture dimension from config; tried attributes: {names}")

        num_hidden_layers = _resolve("num_hidden_layers", "n_layers")
        num_attention_heads = _resolve("num_attention_heads", "n_heads")
        hidden_size = _resolve("hidden_size", "d_model")
        return num_hidden_layers, num_attention_heads, hidden_size
    ...
    def inject_adapter(self, model: nn.Module, adapter_name: str):
        ...
        num_hidden_layers, num_attention_heads, hidden_size = self._get_arch_dims(self.model.config)
        head_dim = hidden_size // num_attention_heads
        A_shape = (hidden_size, peft_config.r)
        B_shape = (peft_config.r, head_dim)
        C_shape = (peft_config.r,)
        self.model.lora_C_l = nn.Parameter(torch.zeros((num_hidden_layers,) + C_shape))
        nn.init.kaiming_uniform_(self.model.lora_C_l, a=math.sqrt(5) * peft_config.init_scale)
        self.model.lora_C_h = nn.Parameter(torch.zeros((num_attention_heads,) + C_shape))
        ...
        self.model.lora_Theta = nn.Parameter(torch.zeros((peft_config.embedding_length,) + C_shape))
        nn.init.kaiming_uniform_(self.model.lora_Theta, a=math.sqrt(5) * peft_config.init_scale)

        # Fixed (frozen) Gaussian Fourier embedding of the mask proportion. `GaussianFourierProjection`
        # concatenates sin/cos, so it outputs `2 * embedding_size` features; we pick `embedding_size` so
        # that phi(mask_proportion) has length `embedding_length` to contract with lora_Theta. The random
        # projection weights are created with `requires_grad=False`, so they are drawn once and never change.
        if peft_config.embedding_length % 2 != 0:
            raise ValueError(
                f"`embedding_length` must be even for the Gaussian Fourier embedding "
    ...
    def _check_new_adapter_config(self, config: NALorTaConfig) -> None:
        """
    ...
    def _prepare_model(self, peft_config: NALorTaConfig, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, NALorTaLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
    ...
    def _mark_only_adapters_as_trainable(self, model: nn.Module) -> None:
        ...
        self.model.lora_Theta.requires_grad = True
        # lora_phi holds the fixed Gaussian Fourier projection; keep it frozen so its random weights never change.
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, NALorTaLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, NALorTaLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        names_with_hooks = []
        for name, module in self.model.named_modules():
            if isinstance(module, NALorTaLayer) or isinstance(module, NALorTaLinear):
    ...
    def _unload_and_optionally_merge(
        ...
        modules_to_process = []
        for name, module in self.model.named_modules():
            if isinstance(module, (NALorTaLayer, NALorTaLinear)):
        ...
        for param_name in ["lora_A", "lora_B", "lora_C_l", "lora_C_h", "lora_C_m", "lora_Theta", "lora_phi"]:
    ...
    def merge_and_unload(
        ...
        r"""
        This method merges the NALoRTA adapters into the base model and unloads them. This is useful when you want to use
        ...
        >>> from peft import NALorTaModel, NALorTaConfig

        >>> base_model = AutoModelForCausalLM.from_pretrained("tiiuae/falcon-40b")
        >>> nalorta_config = NALorTaConfig(...)
        >>> model = NALorTaModel(base_model, nalorta_config)
